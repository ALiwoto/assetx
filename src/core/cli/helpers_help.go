...
func PrintSearchHelp(writer io.Writer) {
	...
	_, _ = fmt.Fprintf(writer, "  --timeout duration  Maximum API wait time (default: %s)\n", appRunner.DefaultSearchTimeout)
	_, _ = fmt.Fprintf(writer, "  --progress-interval duration  Stderr progress interval; 0 disables it (default: %s)\n", appRunner.DefaultSearchProgressInterval)
	_, _ = fmt.Fprintln(writer)
...
func PrintConvertTGSHelp(writer io.Writer) {
	_, _ = fmt.Fprintln(writer, "assetx convert-tgs - convert a Telegram emoji/sticker file to a sprite PNG.")
	_, _ = fmt.Fprintln(writer)
	_, _ = fmt.Fprintln(writer, "Usage:")
	_, _ = fmt.Fprintln(writer, "  assetx convert-tgs --in path/to/input.tgs --out path/to/output.png [options]")
	_, _ = fmt.Fprintln(writer)
	_, _ = fmt.Fprintln(writer, "Required:")
	_, _ = fmt.Fprintln(writer, "  --in path           Input Telegram .tgs/.webm emoji or sticker path")
	_, _ = fmt.Fprintln(writer, "  --out path          Output path ending in .png")
	_, _ = fmt.Fprintln(writer)
	_, _ = fmt.Fprintln(writer, "Options:")
